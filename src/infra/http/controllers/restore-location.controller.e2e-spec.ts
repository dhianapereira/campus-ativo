...
describe('Restore location (E2E)', () => {
  ...
  test('[PATCH] /locations/:id/restore', async () => {
    ...
    const response = await request(app.getHttpServer())
      .patch(`/locations/${locationId}/restore`)
    ...
    expect(locationOnDatabase?.deletedAt).toBeNull()
  })

  test('[PATCH] /locations/:id/restore (location not found)', async () => {
    const user = await userFactory.makePrismaUser({
      role: UserRole.MANAGER,
    })
    ...
    const response = await request(app.getHttpServer())
      .patch('/locations/non-existent-id/restore')
      .set('Authorization', `Bearer ${accessToken}`)

    expect(response.statusCode).toBe(400)
  })

  test('[PATCH] /locations/:id/restore (as reporter - should be forbidden)', async () => {
    const user = await userFactory.makePrismaUser({
