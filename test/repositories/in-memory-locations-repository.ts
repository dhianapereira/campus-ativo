import { LocationsRepository, FetchLocationsParams } from '@/domain/maintenance-problems/application/repositories/locations-repository'
...
export class InMemoryLocationsRepository implements LocationsRepository {
  ...
  async findMany({ page, query, isActive, includeDeleted = false }: FetchLocationsParams) {
    ...
    if (query) {
      ...
      locations = locations.filter(location => {
        ...
        const descriptionMatch = location.description?.toLowerCase().includes(lowerQuery) ?? false
  ...
  async save(location: Location) {
    const itemIndex = this.items.findIndex((item) => item.id.equals(location.id))
